...
export default function IssuesTable() {
  const { issues, filteredIssues, setFilters } = useIssues();
          ...
          <tr>
            <th className="w-8 px-4 py-3">
              <input 
                ...
                onChange={(e) => {
                  setSelectedRows(e.target.checked ? issues.map(i => i.id) : []);
                }}
              />
            </th>
            ...
            <th className="px-4 py-3 text-left text-sm font-medium text-white/60">Team</th>
            ...
            <tr key={issue.id} className="hover:bg-white/5">
              ...
              <td className="px-4 py-3 text-sm">
                <div className="flex items-center gap-2">
                  <span className={cn("px-2 py-1 rounded-full text-xs", getSentimentColor(issue.sentiment))}>
                    {issue.sentiment}
                  </span>
